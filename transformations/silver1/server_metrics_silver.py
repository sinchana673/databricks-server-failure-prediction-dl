...
@dp.table(name="server_metrics_quarantine")
def server_metrics_quarantine():

    df_stream = spark.readStream.table("server_metrics_bronze")

    df_invalid = df_stream.filter(
        (col("cpu").isNull()) | (col("cpu") < 0) | (col("cpu") > 100) |
        (col("memory").isNull()) | (col("memory") < 0) | (col("memory") > 100) |
        (col("disk").isNull()) | (col("disk") < 0) | (col("disk") > 100)
    )
